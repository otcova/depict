...
fn request_animation_frame(f: &Closure<dyn FnMut()>) -> Result<(), String> { web_sys::window()
        .ok_or("Couldn't get window")?
        .request_animation_frame(f.as_ref().unchecked_ref())
        .map_err(|e| format!("Request animation frame failed. {:?}", e))?;
    Ok(())
}
...
pub fn start_animation_loop<F>(mut draw_loop: F) -> Result<(), String>
where
